...
`origin` corporativa é string/null e identifica a baseline embutida ou uma fonte externa explicitamente configurada. `path` é snapshot relativo/null; no uso normal das baselines da skill permanece nulo, pois o projeto registra apenas revisão e hash, sem copiar o documento normativo. `revision` é versão/commit identificável e hash representa bytes reais; hash não autentica aprovação. Fontes corporativas ausentes continuam explicitadas com null. Links de consumo locais ficam dentro da raiz documental, sem caminhos de sessão. Snapshot só é permitido para substituição externa autorizada ou consumidor sem acesso à skill, sempre com origem preservada.
